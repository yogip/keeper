...
`
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				version: 1,
				decoder: func(chipertext []byte) ([]byte, error) {
		...
		{
			name: "Test with error in readPrivateKey",
			args: args{
				version: 1,
				decoder: func(chipertext []byte) ([]byte, error) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := NewEncryptionManager(tt.args.version, "/tmp", tt.args.decoder)
			if tt.wantErr != nil {
...
`
	...
	manager, err := NewEncryptionManager(
		1,
		"/tmp",
		func(chipertext []byte) ([]byte, error) {
			return chipertext, nil
		},
	)
	require.NoError(t, err)
	...
	chiphertext, dataKey, err := manager.Encrypt([]byte("plain text"))
	require.NoError(t, err)

	got, err := manager.Decrypt(chiphertext, dataKey)
	require.NoError(t, err)
